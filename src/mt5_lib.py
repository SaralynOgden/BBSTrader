...
def place_order(order_type, symbol, volume, stop_loss, take_profit, comment, stop_price, direct=False):
    ...
    if direct:
        ...
        if result[0] == 0:
            place_order(order_type, symbol, volume, stop_price, stop_loss, take_profit, comment, True)
        else:
            print(f"Order Code: {result[0]}. Code descriptions: https://www.mql5.com/en/docs/constants/errorswarnings/enum_trade_return_codes")
